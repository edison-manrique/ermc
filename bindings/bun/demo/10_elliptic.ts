/**
 * ERMC Demo 10 — Curvas Elípticas y² = x³ + 7 (mod p)
 * Descubre: Leyes de grupo EC, multiplicación escalar y endomorfismo GLV
 */

import { getElliptic, type EcPoint } from "../src/elliptic";
...
export async function run() {
  ...
  console.log("\n╔══════════════════════════════════════════════════════╗");
  console.log("║  Demo 10: Curvas Elípticas y²=x³+7 mod p (v0.4.0)  ║");
  console.log("╚══════════════════════════════════════════════════════╝\n");
  ...
  if (!G) { console.log("No se encontró punto en la curva."); return; }
  ...
  const lambda = mod.mul(num, mod.inverse(den, P)!, P);
  const x2G_formula = mod.sub(mod.sub(mod.mul(lambda, lambda, P), x, P), x, P);
  console.log(`\nLey diferencial λ(2G): ${lambda}`);
  console.log(`  x(2G) fórmula  = ${x2G_formula}`);
  console.log(`  x(2G) scalarMul = ${twoG.x}`);
  console.log(`  Coinciden: ${x2G_formula === twoG.x} ✓`);

  console.log("\n✓ Demo 10 completada — Curvas Elípticas F_p\n");
}
