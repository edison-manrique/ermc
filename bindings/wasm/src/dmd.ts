...
/**
 * Dynamic Mode Decomposition (DMD en WebAssembly)
...
export class DMD {
   ...
   */
  public static dominantFrequency(snapshots: number[][], dt: number): number {
    ...
    const result = getWasmExports().ermc_dmd_dominant_frequency(
      ptr,
      nSensors,
      nSnaps,
      dt
    );
    wasmFree(ptr, flat.length * 8);
    return result;
  }
}
